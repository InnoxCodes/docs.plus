...
- **Desktop TOC tick rail.** Session-only 32px rail in the pad row so docked chat can go full width. Do not persist rail width. Do not export `TocTickRail` from `toc/index.ts`. Do not put the rail beside the wide TOC (that kills full-width chat). Do not inset rail height for the sash. Do not add `SideContinuum`. Do not hide the preview clone behind a skeleton. Do not drop media because it is taller than the card. Do not drop a whole next heading rank if that leaves a short centered stack. Do not always-center the spy tick. Rail spy tick is `bg-primary`; wide TOC spy stays `menu-focus` / `base-300`. Preview is an L1 clone card, not the house Tooltip. See [apps/webapp/CLAUDE.md](apps/webapp/CLAUDE.md) §TOC And Heading Actions and `CONTEXT.md` §Pad outline.
