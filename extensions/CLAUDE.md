...
- Composer link dialog paths keep the soft-keyboard state. Snapshot `keyboardWasOpenAtOpen` at open. Preview tap, Save, Remove, Cancel, dismiss, and Back keep that state: open stays open, closed stays closed. They move focus back to the composer only when the keyboard was open. Add and edit open are the exception: they always focus the URL field synchronously in the opening tap, so a closed keyboard opens. Cancel or Back from an edit that came from a preview moves focus to the composer in the same tap when the keyboard was open. Never call the pad's `previewHyperlink.dismissSoftKeyboard`.
- The `ComposerLinkModalShell` focusin trap ignores focus on the composer editor (`[data-chat-composer-surface] .ProseMirror`), and it turns off while the shell exits (`useIsPresent`). AnimatePresence keeps two shells mounted during the exit tween, and both refocus paths (close and return to preview) must reach the composer. Without this, a trap pulls focus to a card button. On edit from a preview, the exiting preview trap would also take focus from the URL field.
...
- The internal-link chip renders two ways, mirroring the existing split: imperative `createInternalLinkChip` (desktop `previewShared.ts`/`previewHyperlink.ts`) + React `<InternalLinkChip>` (mobile `LinkPreviewSheet`/`linkPreviewActions`, composer `ComposerLinkPreviewDialog`). The primary action is "Go to destination"/"Go". Copy always yields the raw canonical href. The chip style is `.internal-link-chip` in `document-styles.scss` (token-based, light + dark).
